...
▸ **getDocument**(`id`, `options?`): `Promise`<``null`` \| [`IDocument`](IDocument.md)\>
...
`Promise`<``null`` \| [`IDocument`](IDocument.md)\>
...
| null>} 一个 Promise，在片段数据被检索后，片段数据被检索。
...
▸ **getSnapshot**(`id`, `options?`): `Promise`<``null`` \| [`ISnapshot`](ISnapshot.md)<`S`\>\>
