...
**`property`** {string} fieldName 计算字段的字段名

**`property`** {string} formula 计算字段的公式
...
**`property`** {string} sourceName 数据透视表源字段名称

**`property`** {string} calcItemName 数据透视表源字段计算项名称

**`property`** {string} formula 数据透视表源字段计算项公式

**`property`** {number} priority 数据透视表源字段计算项优先级
...
**`property`** {GC.Pivot.DateGroupType} by 日期分组信息的日期单位

**`property`** {Date} [start] 分组的开始日期（可选）

**`property`** {Date} [end] 分组的结束日期（可选）

**`property`** {number} [groupInterval] 分组的时间间隔（可选）
...
**`property`** {GC.Pivot.PivotDataType} dataType 字段对应的源数据类型

**`property`** {GC.Spread.Pivot.PivotTableFieldType} pivotArea 字段类型，影响字段在不同区域的表现

**`property`** {number} pivotIndex 字段在区域中的索引
...
**`property`** {object}  numberGroup 数值分组信息

**`property`** {number}  [numberGroup.start] 数值分组的起始值（可选）

**`property`** {number}  [numberGroup.end] 数值分组的结束值（可选）

**`property`** {number}  numberGroup.by 数值分组的步长
...
**`property`** {GC.Pivot.IValueInfo} valueInfo 节点的值字段信息。
...
**`property`** {GC.Spread.Pivot.IPivotArea} pivotArea 透视表中的透视表区域。

**`property`** {GC.Spread.Sheets.Style}  style 用于透视表区域的样式。
...
**`property`** {number} row 表示透视表开始的行

**`property`** {number} col 表示透视表开始的列

**`property`** {number} [sheetName] 表示透视表所在的sheet
...
**`property`** {GC.Spread.Sheets.Range} page 表示透视表的过滤区域范围

**`property`** {GC.Spread.Sheets.Range} content 表示透视表的内容区域范围
...
**`property`** {GC.Pivot.IDataPosition} [valuePosition] 透视表的值位置信息

**`property`** {number[]} [pivotTablePosition] 透视表的位置

**`property`** {GC.Spread.Pivot.ISerializeFieldInfo[][]} [fieldsInfo] 透视表的字段信息

**`property`** {GC.Spread.Pivot.IPivotStyle} [styles] 透视表的区域样式
...
**`property`** {GC.Pivot.PivotSourceFieldType} [fieldType] 透视表字段类型是计算字段还是分组字段
...
**`property`** {string} originFieldName 源字段的名称。

**`property`** {object} textGroup 文本组信息

**`property`** {string} textGroup.fieldName 组字段的名称。
