...
- [TimeScaleAutoFitMode](GC.Spread.Sheets.GanttSheet.md#timescaleautofitmode)
...
**`property`** {GC.Spread.Sheets.GanttSheet.DurationUnit} [unit] 表示值的单位。如果未指定，将使用{@link GC.Spread.Sheets.GanttSheet.CalendarSettings}中的默认单位。
...
| `defaultMode?` | [`TaskScheduleMode`](GC.Spread.Sheets.GanttSheet.md#taskschedulemode) | 默认任务模式。 |
...
### <a id="timescaleautofitmode" name="timescaleautofitmode"></a> TimeScaleAutoFitMode

Ƭ **TimeScaleAutoFitMode**: ``"none"`` \| ``"project"``

**`description`** 表示时间刻度的自动适应模式。
