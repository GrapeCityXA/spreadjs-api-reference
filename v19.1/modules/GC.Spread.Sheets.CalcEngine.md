...
- [IGoalSeekOptions](../interfaces/GC.Spread.Sheets.CalcEngine.IGoalSeekOptions.md)
- [IGoalSeekStepInfo](../interfaces/GC.Spread.Sheets.CalcEngine.IGoalSeekStepInfo.md)

### Functions

- [evaluateFormula](GC.Spread.Sheets.CalcEngine.md#evaluateformula)
- [evaluateFormulaAsync](GC.Spread.Sheets.CalcEngine.md#evaluateformulaasync)
...
| `baseColumn?` | `number` | 公式的基准列索引。 |
| `useR1C1?` | `boolean` | 是否使用r1c1引用样式。 |
| `preserveArrayResult?` | `boolean` | 当结果为数组时是否返回数组。如果为false或未定义则返回单个值。 |
...
### <a id="evaluateformulaasync" name="evaluateformulaasync"></a> evaluateFormulaAsync

▸ **evaluateFormulaAsync**(`context`, `formula`, `baseRow?`, `baseColumn?`): `Promise`<`any`\>

计算指定的公式，并返回一个包含公式计算结果的 Promise。
...
sheet = spread.getSheet(0);
sheet.setValue(0, 0, 1);
// 使用 evaluateFormulaAsync() 方法在不设置工作表单元格公式的情况下计算公式
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `context` | `Object` | 计算上下文；一般情况下，您应该使用活动工作表对象。 |
| `formula` | `string` | 公式字符串。 |
| `baseRow?` | `number` | 公式的基准行索引。 |
| `baseColumn?` | `number` | 公式的基准列索引。 |
...
▸ **goalSeek**(`changingSheet`, `changingRow`, `changingColumn`, `formulaSheet`, `formulaRow`, `formulaColumn`, `desiredResult`, `options?`): `boolean` \| `Promise`<`boolean`\>
...
| `options?` | [`IGoalSeekOptions`](../interfaces/GC.Spread.Sheets.CalcEngine.IGoalSeekOptions.md) | - |
...
| `baseRow?` | `number` | 公式的基准行索引。 |
| `baseCol?` | `number` | 公式的基准列索引。 |
| `rangeReferenceRelative?` | [`RangeReferenceRelative`](../enums/GC.Spread.Sheets.CalcEngine.RangeReferenceRelative.md) | 范围引用是相对还是绝对。 |
| `useR1C1?` | `boolean` | 是否使用r1c1引用样式。 |
