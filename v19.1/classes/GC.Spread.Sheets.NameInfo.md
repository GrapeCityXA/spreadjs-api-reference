...
| `comment?` | `string` | 自定义表达式注释。 |
| `isReadOnly?` | `boolean` | 自定义表达式是否可编辑。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `name?` | `string` | 自定义名称的新名称，所有引用的公式都将更新。 |
| `formula?` | `string` | 自定义名称的新公式，所有引用的单元格都将重新计算。 |
| `baseRow?` | `number` | 新公式的行索引。 |
| `baseCol?` | `number` | 新公式的列索引。 |
| `comment?` | `string` | 新的自定义注释。 |
