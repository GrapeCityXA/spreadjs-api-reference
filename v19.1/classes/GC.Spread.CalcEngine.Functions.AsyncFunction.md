...
- [supportCalcWorker](GC.Spread.CalcEngine.Functions.AsyncFunction.md#supportcalcworker)
...
| :------ | :------ | :------ |
| `name` | `string` | 函数名称。 |
| `minArgs?` | `number` | 函数的最小参数数量。 |
| `maxArgs?` | `number` | 函数的最大参数数量。 |
...
### <a id="supportcalcworker" name="supportcalcworker"></a> supportCalcWorker

▸ **supportCalcWorker**(): `boolean`

指示此函数是否可在 Calc Worker 中运行。
如果为 true，则函数实现将被序列化并
在 Calc Worker 内执行。
...
如果函数支持在 Calc Worker 中运行，则返回 `true`；否则返回 `false`。

#### Inherited from

[Function](GC.Spread.CalcEngine.Functions.Function.md).[supportCalcWorker](GC.Spread.CalcEngine.Functions.Function.md#supportcalcworker)
