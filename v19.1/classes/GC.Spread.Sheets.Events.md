...
- [CollaborationEndRedo](GC.Spread.Sheets.Events.md#collaborationendredo)
- [CollaborationEndUndo](GC.Spread.Sheets.Events.md#collaborationendundo)
- [CollaborationStartRedo](GC.Spread.Sheets.Events.md#collaborationstartredo)
- [CollaborationStartUndo](GC.Spread.Sheets.Events.md#collaborationstartundo)
...
- [DataFetchCompleted](GC.Spread.Sheets.Events.md#datafetchcompleted)
...
- [PermissionChanged](GC.Spread.Sheets.Events.md#permissionchanged)
...
- [PivotTableChanging](GC.Spread.Sheets.Events.md#pivottablechanging)
...
- [ThreadedCommentChanged](GC.Spread.Sheets.Events.md#threadedcommentchanged)
- [ThreadedCommentChanging](GC.Spread.Sheets.Events.md#threadedcommentchanging)
...
- [UserMentioned](GC.Spread.Sheets.Events.md#usermentioned)
- [UserMentioning](GC.Spread.Sheets.Events.md#usermentioning)
...
**`param`** *{@link GC.Spread.Sheets.Worksheet}* `oldSheet` 旧工作表。

**`param`** *{@link GC.Spread.Sheets.Worksheet}* `newSheet` 新工作表。
...
**`param`** *{@link GC.Spread.Sheets.Worksheet}* `sheet` 触发事件的工作表。
...
**`param`** *{@link GC.Spread.Sheets.SheetArea}* `sheetArea` 包含单元格的工作表区域。
...
**`param`** *{@link GC.Spread.Sheets.SheetArea}* `sheetArea` 单元格的工作表区域。
...
**`param`** *{@link GC.Spread.Sheets.SheetArea}* `sheetArea` 被点击单元格所在的工作表区域。
...
**`param`** *{@link GC.Spread.Sheets.ClipboardActionType}* `action` 指示当前操作的类型。
...
**`param`** *{@link GC.Spread.Sheets.Range}* `cellRange` 被粘贴的范围。

**`param`** *{@link GC.Spread.Sheets.ClipboardPasteOptions}* `pasteOption` 指示从剪贴板粘贴的数据类型：值、格式或公式。
...
**`param`** *{@link GC.Spread.Sheets.Worksheet}* `fromSheet` 数据范围的源工作表。

**`param`** *{@link GC.Spread.Sheets.Range}* `[fromRange]` 复制或剪切范围时粘贴的源范围。

**`param`** *{@link GC.Spread.Sheets.ClipboardActionType}* `action` 指示当前操作的类型。

**`param`** *Array* `objects` 粘贴的浮动对象，包含图片、自定义浮动对象、切片器、图表和形状。

**`param`** *{@link GC.Spread.Sheets.InsertShiftCell}* `[shiftCells]` 如果未移动单元格，则将未定义。
...
**`param`** *{@link GC.Spread.Sheets.Range}* `cellRange` 要粘贴的范围。
...
### <a id="collaborationendredo" name="collaborationendredo"></a> CollaborationEndRedo
...
### <a id="collaborationendundo" name="collaborationendundo"></a> CollaborationEndUndo
...
### <a id="collaborationstartredo" name="collaborationstartredo"></a> CollaborationStartRedo
...
### <a id="collaborationstartundo" name="collaborationstartundo"></a> CollaborationStartUndo
...
**`param`** *{@link GC.Spread.Sheets.SheetArea}* `sheetArea` 列的工作表区域。

**`param`** *string* `propertyName` 列的属性名称。

**`param`** *Object* `oldValue` 该属性的旧值。

**`param`** *Object* `newValue` 该属性的新值。

**`param`** *number* `[count]` 受影响的列数。
...
**`param`** *boolean* `cancel` 是否应取消本次操作。
...
**`param`** *{@link GC.Spread.Sheets.Comments.Comment}* `comment` 触发事件的注释。
...
**`param`** *{@link GC.Spread.Sheets.Comments.Comment}* `comment` 已删除的注释。
...
**`param`** *{@link GC.Spread.Sheets.Comments.Comment}* `comment` 正在删除的注释。
...
### <a id="datafetchcompleted" name="datafetchcompleted"></a> DataFetchCompleted
...
**`param`** *{@link GC.Spread.Sheets.Workbook}* `spread` 触发事件的工作簿。
...
**`param`** *{@link GC.Spread.Sheets.CopyToOptions}* `copyOption` 拖放操作的CopyOption值。
...
**`param`** *{@link GC.Spread.Sheets.Range}* `fillRange` 用于填充操作的范围。

**`param`** *{@link GC.Spread.Sheets.Fill.AutoFillType}* `autoFillType` 用于填充操作的AutoFillType值。

**`param`** *{@link GC.Spread.Sheets.Fill.FillDirection}* `fillDirection` 填充的方向。
...
**`param`** *{@link GC.Spread.Sheets.Range}* `mergeRange` 将要合并的范围。
...
**`param`** *{@link GC.Spread.Sheets.EditorStatus}* `oldStatus` 编辑器的旧状态。

**`param`** *{@link GC.Spread.Sheets.EditorStatus}* `newStatus` 编辑器的新状态。
...
**`param`** *{@link GC.Spread.Sheets.FloatingObjects.FloatingObject}* `floatingObject` 触发事件的浮动对象。
...
**`param`** *{@link GC.Spread.Sheets.FloatingObjects.FloatingObject}* `floatingObject` 触发事件的自定义浮动对象。
...
**`param`** *{@link GC.Spread.Sheets.FloatingObjects.FloatingObject}* `floatingObject` 已被移除的浮动对象。
...
**`param`** *{@link GC.Spread.Sheets.FloatingObjects.FloatingObject}* `floatingObject` 正在被移除的浮动对象。
...
**`param`** *{@link GC.Spread.Sheets.Shapes.Shape}* `shape` 触发事件的形状。
...
**`param`** *{@link GC.Spread.Sheets.InvalidOperationType}* `invalidType` 指示哪个操作无效。

**`param`** *string* `message` 无效操作的描述。

**`param`** *{@link GC.Spread.Sheets.Range}* `[fillRange]` 当invalidType为dragFill时，用于填充操作的范围。
...
### <a id="permissionchanged" name="permissionchanged"></a> PermissionChanged
...
**`param`** *{@link GC.Spread.Sheets.Collaboration.IPermission}* `permission` 触发该事件的权限信息（注：原文注释标注为“工作表”，疑似笔误，结合参数名及类型，应为“权限信息”）。

**`example`**
```javascript
//此示例使用 PermissionChanged 事件。
...
**`param`** *{@link GC.Spread.Sheets.FloatingObjects.Picture}* `picture` 触发事件的图片。
...
### <a id="pivottablechanging" name="pivottablechanging"></a> PivotTableChanging

• `Static` **PivotTableChanging**: `string`

在 PivotTable fieldChanged 之前触发。

**`name`** GC.Spread.Sheets.Worksheet#PivotTableChanging

**`param`** *string* `pivotTableName` PivotTable 的名称。

**`param`** *string* `type` 具体操作名称（fieldChanging）。

**`param`** *string* `[fieldName]` 正在变更的 fieldName。

**`param`** *string* `[sourceName]` 正在变更的字段 sourceName。

**`param`** *number* `[newArea]` 新的 PivotTable 字段类型（fieldChanging）。

**`param`** *number* `[newIndex]` 新的 PivotTable 字段索引（fieldChanging）。

**`param`** *boolean* `[autoGroupDateField]` 是否自动对当前修改的日期字段进行分组。

**`param`** *boolean* `[cancel]` 是否取消当前操作。
...
**`param`** *{@link GC.Spread.Sheets.RangeChangedAction}* `action` 引发RangeChanged事件的操作类型。
...
**`param`** *{@link GC.Spread.Sheets.Range}* `range` 自动排序的范围。

**`param`** *{@link GC.Spread.Sheets.GroupSort}* `groupSort` 排序时使用的groupSort级别，默认情况下如果包含组则使用组级别，如果不包含组则使用平面级别。
...
**`param`** *{@link GC.Spread.Report.ReportSheet}* `sheet` 触发事件的报表。
...
**`param`** *{@link GC.Spread.Sheets.SheetArea}* `sheetArea` 行所在的工作表区域。
...
**`param`** *{@link GC.Spread.Sheets.TableSheet.TableSheet}* `sheet` 触发事件的计算表。

**`param`** *string* `sheetName` 计算表的名称。

**`param`** *{@link GC.Spread.Sheets.TableSheet.ActionType}* `actionType` 行操作类型。
...
**`param`** *{@link GC.Spread.Sheets.Shapes.Shape}* `shape` 已被移除的形状。
...
**`param`** *{@link GC.Spread.Sheets.Shapes.Shape}* `shape` 正在被移除的形状。
...
**`param`** *string* `propertyName` 具体的操作名称。

**`param`** *number* `sheetIndex` 工作表索引，与 Worksheet 集合或 SheetTab 集合相关。

**`param`** *number* `sheetPosition` 工作表位置，与包含所有 Worksheet 和 SheetTab 的组合集合相关。

**`param`** *boolean* `oldValue` 工作表变更前的属性值（isVisible、isSelected 等）。

**`param`** *boolean* `newValue` 工作表变更后的属性值（isVisible、isSelected 等）。

**`param`** *boolean* `cancel` 取消当前操作。
...
**`param`** *{@link GC.Spread.Sheets.Slicers.ISlicer}* `slicer` 触发事件的切片器。
...
**`param`** *{@link GC.Spread.Sheets.Sparklines.Sparkline}* `sparkline` 属性已更改的迷你图。
...
**`param`** *{@link GC.Spread.Sheets.Tables.Table}* `table` 插入/删除行的表格。
...
**`param`** *{@link GC.Spread.Sheets.Tables.Table}* `table` 要自动筛选的表格列。
...
**`param`** *{@link GC.Spread.Sheets.Tables.Table}* `table` 被调整大小的表格。

**`param`** *{@link GC.Spread.Sheets.Range}* `oldRange` 调整前的表格范围。

**`param`** *{@link GC.Spread.Sheets.Range}* `newRange` 调整后的表格范围。
...
**`param`** *{@link GC.Spread.Sheets.Tables.Table}* `table` 正在调整大小的表格。
...
### <a id="threadedcommentchanged" name="threadedcommentchanged"></a> ThreadedCommentChanged

• `Static` **ThreadedCommentChanged**: `string`

当任意评论发生变更时触发。

**`name`** GC.Spread.Sheets.Worksheet#ThreadedCommentChanged

**`param`** *{@link GC.Spread.Sheets.Worksheet}* `sheet` 触发该事件的工作表。

**`param`** *string* `sheetName` 该工作表的名称。

**`param`** *{@link GC.Spread.Sheets.ThreadedComments.ThreadedComment}* `threadedComment` 触发该事件的评论。

**`param`** *string* `propertyName` 发生变更的评论属性名称。

**`param`** *number* `[replyIndex]` 发生变更的评论对应的回复索引。若本次变更与任何回复无关，则该值为 undefined。

**`param`** *any* `oldValue` 该属性的旧值。

**`param`** *any* `newValue` 该属性的新值。

**`example`**
```javascript
//此示例使用 ThreadedCommentChanged 事件。
activeSheet.bind(GC.Spread.Sheets.Events.ThreadedCommentChanged, function (e, info) {
    alert("changed");
});
```
...
### <a id="threadedcommentchanging" name="threadedcommentchanging"></a> ThreadedCommentChanging

• `Static` **ThreadedCommentChanging**: `string`

当任意评论正在发生变更时触发。

**`name`** GC.Spread.Sheets.Worksheet#ThreadedCommentChanging

**`param`** *{@link GC.Spread.Sheets.Worksheet}* `sheet` 触发该事件的工作表。

**`param`** *string* `sheetName` 该工作表的名称。

**`param`** *{@link GC.Spread.Sheets.ThreadedComments.ThreadedComment}* `threadedComment` 即将发生变更的评论。

**`param`** *string* `propertyName` 正在发生变更的评论属性名称。

**`param`** *number* `[replyIndex]` 正在发生变更的评论对应的回复索引。若本次变更与任何回复无关，则该值为 undefined。

**`param`** *any* `oldValue` 该属性的旧值。

**`param`** *any* `newValue` 该属性的新值。

**`param`** *boolean* `cancel` 一个用于指示是否应取消本次操作的值。

**`example`**
```javascript
//此示例使用 ThreadedCommentChanging 事件。
activeSheet.bind(GC.Spread.Sheets.Events.ThreadedCommentChanging, function (e, info) {
    if(info.replyIndex === undefined){
         info.cancel = true;
    }
});
```
...
**`param`** *number* `x` 水平位置的<i>x</i>坐标。

**`param`** *number* `y` 垂直位置的<i>y</i>坐标。

**`param`** *boolean* `handled` 如果为`true`，则阻止触摸工具栏弹出；否则，工具栏将显示在默认位置。
...
### <a id="usermentioned" name="usermentioned"></a> UserMentioned

• `Static` **UserMentioned**: `string`

当任意用户被提及（@）时触发。

**`name`** GC.Spread.Sheets.Worksheet#UserMentioned

**`param`** *{@link GC.Spread.Sheets.Worksheet}* `sheet` 触发该事件的工作表。

**`param`** *string* `sheetName` 该工作表的名称。

**`param`** *Object* `context` 提及操作发生的上下文信息。

**`param`** *{@link GC.Spread.Sheets.MentionContextType}* `context.type` 提及操作的上下文类型。

**`param`** 提及了用户的评论（可选）。

**`param`** 发生提及操作的评论对应的回复索引（可选）。

**`param`** *string[]* `userIds` 被提及的用户ID数组。

**`example`**
```javascript
//此示例使用 UserMentioned 事件。
sheet.bind(GC.Spread.Sheets.Events.UserMentioned, function (e, info) {
    console.log(info.userIds);
});
```

___

### <a id="usermentioning" name="usermentioning"></a> UserMentioning

• `Static` **UserMentioning**: `string`

当即将触发提及时发生。将 cancel 设置为 true 可阻止提及。
...
**`param`** *Object* `context` 提及触发上下文。

**`param`** *{@link GC.Spread.Sheets.MentionContextType}* `context.type` 提及上下文类型。

**`param`** 触发提及的线程批注。

**`param`** *boolean* `cancel` 设置为 true 以取消提及。

**`example`**
```javascript
// 此示例使用 UserMentioning 事件。
...
**`param`** *{@link GC.Spread.Sheets.DataValidation.DefaultDataValidator}* `validator` 导致错误的数据验证器。

**`param`** *{@link GC.Spread.Sheets.DataValidation.DataValidationResult}* `validationResult` 用户可以设置的处理错误的策略。
