...
- [getBindingSource](GC.Spread.Sheets.Tables.Table.md#getbindingsource)
...
获取或设置表格的allowAutoExpandState属性。该属性用于控制在工作表中操作数据时，表格是否允许自动调整大小。

**`example`**
```javascript
var activeSheet = spread.getActiveSheet();
var table = activeSheet.tables.add("table1", 1, 1, 10, 5, GC.Spread.Sheets.Tables.TableThemes.medium2);
console.log(table.allowAutoExpand()); // true
console.log(table.range().rowCount); // 10
activeSheet.setActiveCell(11, 2);
activeSheet.startEdit(true, "Spreadjs");
activeSheet.endEdit();
console.log(table.range().rowCount); // 11
table.allowAutoExpand(false);
console.log(table.allowAutoExpand());
activeSheet.setActiveCell(12, 2);
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `allowAutoExpandState?` | `boolean` | 表格的自动扩展状态。 |
...
| `value?` | [`ITableLayoutStyle`](../interfaces/GC.Spread.Sheets.Tables.ITableLayoutStyle.md) | 布局样式。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | `boolean` | 是否通过工作表或表格插入/删除行来扩展。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `tableColumnIndex?` | `number` \| `boolean` | 过滤器按钮的表格列索引。 |
| `value?` | `boolean` | 表格列的过滤器按钮是否显示。 |
...
### <a id="getbindingsource" name="getbindingsource"></a> getBindingSource

▸ **getBindingSource**(): ``null`` \| `object` \| [`Table`](GC.Data.Table.md)
...
| `count` | `number` | 要添加的列数。 |
| `isInsertAfter?` | `boolean` | 是否在指定列索引之前插入列。默认情况下，插入到之前。 |
...
| `count` | `number` | 要添加的行数。 |
| `isInsertAfter?` | `boolean` | 是否在指定行索引之前插入行。默认情况下，插入到之前。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | [`ITableLayoutStyle`](../interfaces/GC.Spread.Sheets.Tables.ITableLayoutStyle.md) | 一个命名样式或样式实例。 |
...
// 更新 tableColumn2 的名称和 dataField
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | `boolean` | 是否显示页脚。 |
| `isFooterInserted?` | `boolean` | 该值决定显示页脚的方式，是插入总计行还是覆盖下一行。移除页脚的方式相同。 |
