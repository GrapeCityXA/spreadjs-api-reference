...
- [autoFitMode](GC.Spread.Sheets.GanttSheet.Timescale.md#autofitmode)
...
- [margin](GC.Spread.Sheets.GanttSheet.Timescale.md#margin)
...
• **autoFitMode**: [`TimeScaleAutoFitMode`](../modules/GC.Spread.Sheets.GanttSheet.md#timescaleautofitmode)

获取或设置时间刻度的自动适应模式。设置为 "project" 时，minDate 和 maxDate 会在每次调度后自动调整为项目的实际日期范围。默认值为 "none"。
...
获取或设置时间刻度的边距。该边距会在 minDate 之前和 maxDate 之后，按指定的最小层级单位数量扩展可滚动范围。默认值为 0。
...
| `percentOfViewport?` | `number` | 一个 0 到 1 之间的数值，表示目标日期要滚动到的视口位置的百分比。0 表示视口左侧，1 表示视口右侧。默认为 0。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `adjustTierUnit?` | `boolean` | 一个布尔值，指示是否根据缩放因子将层级的单位调整为合适的值。默认为 true. |
