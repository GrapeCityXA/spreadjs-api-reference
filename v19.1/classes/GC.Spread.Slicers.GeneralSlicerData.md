...
MySlicer.prototype.onFiltered = function () {
    //如果数据已由 Spread.Sheets 表格筛选，则同步状态。
    var slicerData = this.slicerData;
    var exclusiveIndexes = slicerData.getFilteredIndexes(this.columnName);
    var inputs = document.querySelectorAll("#slicerContainer > input");
    inputs.forEach(function(input, i) {
...
| `isPreview?` | `boolean` | 是否设置预览。如果设置为预览，则可以通过 clearPreview() 清除过滤结果，可以通过 inPreview() 检查预览状态。 |
...
| :------ | :------ | :------ |
| `columnName` | `string` | 列名 |
| `range?` | [`ISlicerRangeConditional`](../interfaces/GC.Spread.Slicers.ISlicerRangeConditional.md) | 特定范围，仅支持数字类型值 |
