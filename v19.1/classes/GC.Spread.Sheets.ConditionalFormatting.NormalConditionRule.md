...
- [isPercent](GC.Spread.Sheets.ConditionalFormatting.NormalConditionRule.md#ispercent)
...
• **new NormalConditionRule**(`ruleType`, `ranges`, `style`, `operator`, `value1`, `value2`, `text`, `formula`, `type`, `rank`, `isPercent?`)
...
### <a id="ispercent" name="ispercent"></a> isPercent
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | `boolean` | 排名值是数字还是百分比。 |

#### Returns

`any`

如果未设置值，则返回排名值是数字还是百分比；否则返回数字条件规则。
