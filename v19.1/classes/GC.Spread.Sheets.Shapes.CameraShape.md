...
| `left?` | `number` | 可选参数。形状的<i>x</i>坐标（水平位置）。 |
| `top?` | `number` | 可选参数。形状的<i>y</i>坐标（垂直位置）。 |
| `width?` | `number` | 可选参数。照相机形状的宽度，留空则默认与原始单元格区域的宽度一致。 |
| `height?` | `number` | 可选参数。照相机形状的高度，留空则默认与原始单元格区域的高度一致。 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | `string` \| `Function` | 双击照相机形状时执行的行为（而非跳转到源单元格区域），若为函数则不会序列化。默认命令为'openUrl'。 |
