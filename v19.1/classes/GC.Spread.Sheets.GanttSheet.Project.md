...
- [getTaskByRow](GC.Spread.Sheets.GanttSheet.Project.md#gettaskbyrow)
...
- [insertTasksByRow](GC.Spread.Sheets.GanttSheet.Project.md#inserttasksbyrow)
...
| `level?` | `number` | 可选。一个数字表示要添加的任务级别。使用指定的级别，任务可以作为最后一个任务的子任务、同级任务或上级任务添加。如果未提供，将自动设置级别。 |
...
根据甘特图工作表的指定行索引获取对应的任务。

#### Parameters

| Name | Type | Description |
| :------ | :------ | :------ |
| `rowIndex` | `number` | 甘特图工作表的指定行索引。 |
...
### <a id="inserttasksbyrow" name="inserttasksbyrow"></a> insertTasksByRow

▸ **insertTasksByRow**(`rowIndex`, `data?`, `level?`): [`Task`](GC.Spread.Sheets.GanttSheet.Task.md)[]
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `rowIndex` | `number` | 指定甘特图工作表中要插入任务的目标行索引。 |
| `data?` | `number` \| [`ITaskData`](../modules/GC.Spread.Sheets.GanttSheet.md#itaskdata) \| [`ITaskData`](../modules/GC.Spread.Sheets.GanttSheet.md#itaskdata)[] | - |
| `level?` | `number` | 可选参数。传入数字表示要添加的任务的层级。指定层级后，任务可以作为上一条任务的子任务、同级任务或上级任务进行添加。若未传入此参数，将自动设置任务层级。 |
...
| `ascendingStates?` | `boolean`[] | 指定相关字段的升序状态。 |
