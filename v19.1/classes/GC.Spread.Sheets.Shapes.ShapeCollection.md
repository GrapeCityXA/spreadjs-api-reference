...
| `left?` | `number` | 相机形状的<i>x</i>位置，如果留空则默认值为0。 |
| `top?` | `number` | 相机形状的<i>y</i>位置，如果留空则默认值为0。 |
| `width?` | `number` | 相机形状的宽度，如果留空则默认值应与原始范围宽度相同。 |
| `height?` | `number` | 相机形状的高度，如果留空则默认值应与原始范围高度相同。 |
...
| `beginX?` | `number` | 连接器形状起点的<i>x</i>位置。 |
| `beginY?` | `number` | 连接器形状起点的<i>y</i>位置。 |
| `endX?` | `number` | 连接器形状终点的<i>x</i>位置。 |
| `endY?` | `number` | 连接器形状终点的<i>y</i>位置。 |
...
| `left?` | `number` | 表单控件形状的<i>x</i>位置。 |
| `top?` | `number` | 表单控件形状的<i>y</i>位置。 |
| `width?` | `number` | 表单控件形状的宽度。 |
| `height?` | `number` | 表单控件形状的高度。 |
...
| `name` | `string` | 形状的名称。如果名称为空字符串，将生成一个唯一名称。 |
| `src` | `string` | 图片的源地址。 |
| `left?` | `number` | 图片形状的<i>x</i>位置。 |
| `top?` | `number` | 图片形状的<i>y</i>位置。 |
| `width?` | `number` | 图片形状的宽度。 |
| `height?` | `number` | 图片形状的高度。 |
