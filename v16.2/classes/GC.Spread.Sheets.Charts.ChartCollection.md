...
| Name | Type | Description |
| :------ | :------ | :------ |
| `flag` | `boolean` | 在导入时是否保留不支持图表,默认值为false,如果设置为true,则将其绘制为paintCallBack |
| `paintCallBack?` | [`IPaintCallBack`](../interfaces/GC.Spread.Sheets.Charts.IPaintCallBack.md) | 不支持图表的显示内容函数 |
