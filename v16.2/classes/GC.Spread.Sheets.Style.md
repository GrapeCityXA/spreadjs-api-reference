...
| :------ | :------ | :------ |
| `backColor?` | `string` \| [`IGradientFill`](../interfaces/GC.Spread.Sheets.IGradientFill.md) \| [`IGradientPathFill`](../interfaces/GC.Spread.Sheets.IGradientPathFill.md) \| [`IPatternFill`](../interfaces/GC.Spread.Sheets.IPatternFill.md) | 背景色 |
| `foreColor?` | `string` | 前景色 |
| `hAlign?` | [`HorizontalAlign`](../enums/GC.Spread.Sheets.HorizontalAlign.md) | 水平对齐 |
| `vAlign?` | [`VerticalAlign`](../enums/GC.Spread.Sheets.VerticalAlign.md) | 垂直对齐 |
| `font?` | `string` | 字体 |
| `themeFont?` | `string` | 主题字体 |
| `formatter?` | `string` \| [`GeneralFormatter`](GC.Spread.Formatter.GeneralFormatter.md) | 格式化对象 |
| `borderLeft?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 左边框 |
| `borderTop?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 上边框 |
| `borderRight?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 右边框 |
| `borderBottom?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 下边框 |
| `locked?` | `boolean` | 是否锁定 |
| `textIndent?` | `number` | 缩进量 |
| `wordWrap?` | `boolean` | 是否回行 |
| `shrinkToFit?` | `boolean` | 是否缩放 |
| `backgroundImage?` | `string` | 背景图片 |
| `cellType?` | [`Base`](GC.Spread.Sheets.CellTypes.Base.md) | 单元格类型 |
| `backgroundImageLayout?` | [`ImageLayout`](../enums/GC.Spread.Sheets.ImageLayout.md) | 背景图片布局类型 |
| `tabStop?` | `boolean` | 是否允许用户使用 Tab 键设置焦点 |
| `textDecoration?` | [`TextDecorationType`](../enums/GC.Spread.Sheets.TextDecorationType.md) | 文字装饰类型 |
| `imeMode?` | [`ImeMode`](../enums/GC.Spread.Sheets.ImeMode.md) | IME 模式 |
| `name?` | `string` | 名称 |
| `parentName?` | `string` | 父样式名称 |
| `watermark?` | `string` | 水印文字 |
| `cellPadding?` | `string` | 单元格填充 |
| `labelOptions?` | [`ILabelOptions`](../interfaces/GC.Spread.Sheets.ILabelOptions.md) | 单元格标签选项 |
| `diagonalDown?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 左上角到右下角的对角线 |
| `diagonalUp?` | [`LineBorder`](GC.Spread.Sheets.LineBorder.md) | 左下角到右上角的对角线 |
| `isVerticalText?` | `boolean` | 垂直对齐 |
| `textOrientation?` | `number` | 单元格文字旋转角度 |
| `mask?` | [`IMaskType`](../interfaces/GC.Spread.Sheets.IMaskType.md) | 输入掩码的掩码对象 |
| `applyFont?` | `boolean` | - |
| `applyFill?` | `boolean` | - |
...
**`property`** {GC.Spread.Sheets.ButtonImageType} [imageType] - 特定于按钮的类型(要在按钮中显示的图像类型),默认值为"自定义"为单元格按钮提供一些预定义的类型,自定义允许特定的图标

**`property`** {string} [imageSrc] - 当imageType为自定义时,可以通过imageSrc指定图像(base64)

**`property`** {Object} [imageSize] - 图像的大小,默认值为16px

**`property`** {number} [imageSize.width] - 图像的宽度,默认值为16,单位为px

**`property`** {number} [imageSize.height] - 图像的高度,默认值为16,单位为px

**`property`** {string} [hoverBackColor] - 当按钮可见并启用时,单元格按钮的悬停背景色

**`property`** {string} [buttonBackColor] - 当按钮处于启用状态时,单元格按钮的背景色
