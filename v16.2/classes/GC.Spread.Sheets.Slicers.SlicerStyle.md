...
获取或设置切片器的style of the hovered selected item with data.
...
If no value is set, returns the style of the hovered selected item with data; otherwise, returns the 切片器样式.
...
**`代码示例`**
```
//本示例使用hoveredSelectedItemWithNoDataStyle方法
var datasource = [
...
If no value is set, returns the style of the hovered unselected item with data; otherwise, returns the 切片器样式.
