...
**`property`** {GC.Spread.Sheets.Shapes.ShapeFillType} [type] - 填充类型,它是ShapeFillType枚举或公式(以=开头)

**`property`** {string} [color] - 背景色,它是一个颜色字符串或公式(以=开头)

**`property`** {number | string} [transparency] - 背景色透明度,它是一个介于0和1之间的数字或公式(以=开头)
...
**`property`** {string} [color] - 线条颜色,它是一个颜色字符串或公式(以=开头)

**`property`** {GC.Spread.Sheets.Shapes.PresetLineDashStyle | string} [lineStyle] - 线条类型,它是PresetLineDashStyle枚举或公式(以=开头)

**`property`** {number | string} [width] - 线条宽度,它是一个数字或公式(以=开头)

**`property`** {GC.Spread.Sheets.Shapes.LineCapStyle | string} [capType] - 线帽类型,它是LineCapStyle枚举或公式(以=开头)

**`property`** {GC.Spread.Sheets.Shapes.LineJoinStyle | string} [joinType] - 线联接类型,它是LineJoinStyle枚举或公式(以=开头)

**`property`** {GC.Spread.Sheets.Shapes.CompoundType | string} [compoundType] - 行复合类型,它是compundType枚举或公式(以=)开头

**`property`** {number | string} [transparency] - 线条颜色的透明度,它是介于0和1之间的数字或公式(以=开头)

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadStyle} [beginArrowheadStyle] - 箭头始端样式,它是ArrowheadStyle枚举

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadWidth} [beginArrowheadWidth] - 箭头始端宽度,它是ArrowheadWidth枚举

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadLength} [beginArrowheadLength] - 箭头始端长度,它是ArrowheadLength枚举

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadStyle} [endArrowheadStyle] - 箭头末端样式,它是ArrowheadStyle枚举

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadWidth} [endArrowheadWidth] - 箭头末端宽度,它是ArrowheadWidth枚举

**`property`** {GC.Spread.Sheets.Shapes.ArrowheadLength} [endArrowheadLength] - 箭头末端长度,它是ArrowheadLength枚举
...
**`property`** {string} [color] - 字体颜色,它是一个颜色字符串或公式(以=开头)

**`property`** {number | string} [transparency] - 字体颜色的透明度,它是介于0和1之间的数字或公式(以=开头)

**`property`** {string} [font] - 字体,它是字体字符串或公式(以=开头)
...
**`property`** {GC.Spread.Sheets.VerticalAlign | string} [vAlign] - 文本垂直对齐方式,它是VerticalAlign枚举或公式(以=开头)

**`property`** {GC.Spread.Sheets.HorizontalAlign | string} [hAlign] - 文本水平对齐方式,它是HorizontalAlign枚举或公式(以=开头)

**`property`** {boolean} [resizeToFitText] - 调整形状大小以适合文本大小。
