...
**`property`** {boolean} allowUserDragDrop - 是否允许用户拖放区域数据

**`property`** {boolean} allowUserDragFill - 是否允许用户拖动填充区域

**`property`** {boolean} allowUserZoom -是否在按Ctrl键的同时滚动鼠标滚轮来缩放显示

**`property`** {boolean} allowUserResize - 是否允许用户调整列和行的大小

**`property`** {boolean} allowUndo -是否允许用户撤消编辑

**`property`** {boolean} allowSheetReorder - 用户是否可以在 SpreadJS 组件中重新排序表单

**`property`** {boolean} allowContextMenu - 是否允许用户打开内置上下文菜单

**`property`** {boolean} allowUserDeselect -  是否允许用户在选择中使用取消选择

**`property`** {GC.Spread.Sheets.Fill.AutoFillType} defaultDragFillType - 默认填充类型

**`property`** {boolean} showDragFillSmartTag - 是否显示拖拽填充对话框

**`property`** {boolean} showHorizontalScrollbar - 是否显示水平滚动条

**`property`** {boolean} showVerticalScrollbar - 是否显示垂直滚动条

**`property`** {boolean} scrollbarShowMax - 显示的滚动条是否基于表单中的完整列数和行数

**`property`** {boolean} scrollbarMaxAlign - 滚动条是否与活动表单的最后一行和一列对齐

**`property`** {boolean} tabStripVisible - 是否显示表单选项卡条

**`property`** {number} tabStripRatio - 标签条的宽度占整个水平滚动条宽度的百分比

**`property`** {number} tabStripWidth - 标签条处于左边或右边位置时的宽度默认值和最小值是80

**`property`** {boolean} tabEditable - 是否允许用户编辑工作表标签条

**`property`** {GC.Spread.Sheets.TabStripPosition} tabStripPosition - 标签条的位置，默认是bottom

**`property`** {boolean} newTabVisible - 表单是否显示允许用户插入新表单（WorkSheet）的选项标签

**`property`** {GC.Spread.Sheets.AllSheetsListVisibility} allSheetsListVisible - 工作簿是否显示特殊选项卡,允许用户打开对话框以显示所有表单

**`property`** {boolean} tabNavigationVisible - 是否显示表单选项卡导航

**`property`** {boolean} cutCopyIndicatorVisible - 是否在复制或剪切所选项时显示指示符

**`property`** {string} cutCopyIndicatorBorderColor - 用户剪切或复制所选内容时显示的指示器的边框颜色

**`property`** {string} backColor - 用于表示 SpreadJS 的背景颜色的颜色字符串,例如 "red", "#FFFF00", "rgb(255,0,0)", "Accent 5"等

**`property`** {string} backgroundImage - SpreadJS 的背景图像

**`property`** {GC.Spread.Sheets.ImageLayout} backgroundImageLayout - SpreadJS 的背景图像布局

**`property`** {string} grayAreaBackColor - 用于表示灰色区域的背景色的颜色字符串,例如 "red", "#FFFF00", "rgb(255,0,0)", "Accent 5"等
...
**`property`** {boolean} useTouchLayout - 是否使用触摸布局来显示 SpreadJS

**`property`** {boolean} hideSelection - 是否在 SpreadJS 没有焦点时突出显示选择

**`property`** {GC.Spread.Sheets.ResizeZeroIndicator} resizeZeroIndicator - 将行或列的大小调整为零时的绘制策略

**`property`** {boolean} allowUserEditFormula - 用户是否可以在表单的单元格中编辑公式

**`property`** {boolean} enableFormulaTextbox - 是否在表单中启用公式文本框

**`property`** {GC.Spread.Sheets.AutoFitType} autoFitType - 内容是否格式化以适合单元格或单元格和标题

**`property`** {GC.Spread.Sheets.ReferenceStyle} referenceStyle - 此表单中单元格公式中的单元格和区域引用的样式

**`property`** {boolean} allowDynamicArray - 是否启用动态数组

**`property`** {boolean} iterativeCalculation - 是否启用迭代计算

**`property`** {number} iterativeCalculationMaximumIterations - 迭代计算时最大迭代次数

**`property`** {number} iterativeCalculationMaximumChange - 迭代计算时变化最大量

**`property`** {boolean} calcOnDemand - 是否只在需要时才计算公式

**`property`** {boolean} dynamicReferences - 动态引用时计算公式

**`property`** {boolean} allowCopyPasteExcelStyle - 用户是否可以从表单复制样式然后粘贴到Excel,或者从Excel复制样式然后粘贴到表单

**`property`** {boolean} allowExtendPasteRange - 如果粘贴区域不足以粘贴,是否扩展粘贴区域

**`property`** {GC.Spread.Sheets.CopyPasteHeaderOptions} copyPasteHeaderOptions - 复制或粘贴数据时包含的表头

**`property`** {GC.Spread.Sheets.CalculationMode} calculationMode - 指定工作簿的重新计算模式

**`property`** {boolean} scrollByPixel - 是否启用按像素滚动的精度

**`property`** {number} scrollPixel - 当scrollByPixel为true时,决定每次滚动的像素数最终滚动像素是滚动delta乘以scrollPixel的结果例如,滚动的delta是3,scrollPixel是5,最终滚动的像素是15

**`property`** {boolean} enableAccessibility -是否在电子表格中启用可访问性支持

**`property`** {boolean} allowAutoCreateHyperlink - 是否在表单中启用自动创建超链接

**`property`** {GC.Spread.Sheets.ResizeMode} columnResizeMode - 调整列大小的方法

**`property`** {GC.Spread.Sheets.ResizeMode} rowResizeMode - 调整行大小的方法

**`property`** {Array} customList - 列表供用户自定义拖动填充,优先级匹配此列表中的每个填充每个数组项都是字符串数组的类型

**`property`** {GC.Spread.Sheets.ScrollbarAppearance} scrollbarAppearance - 滚动条的外观,包含外观和移动两个枚举 默认值为外观

**`property`** {boolean} pasteSkipInvisibleRange - 是否粘贴跳过不可见的区域默认为false

**`property`** {boolean} allowAutoExtendFilterRange -是否允许像excel一样自动扩展筛选区域,默认值为false

**`property`** {boolean} allowInvalidFormula - 是否允许输入无效的公式字符串。缺省值为false。

**`property`** {boolean} formulaFormatHint - 是否在公式输入时自动生成格式。默认为True。

**`property`** {GC.Spread.Pivot.PivotAreaReference} pivotAreaReference  - 在选择数据透视表数据区域时，是否自动生成getPivotData公式或单元格引用。默认是getPivotData。
...
| `type` | [`tableSheet`](../enums/GC.Spread.Sheets.SheetType.md#tablesheet) | 要添加的工作表选项卡的类型：集算表 |
...
// 表单的顺序应该是 "Sheet1", "Sheet2", "Sheet3", "Sheet4", "Sheet5"
spread.setSheetCount(5); 

// 表单顺序应该是 "Sheet2", "Sheet3", "Sheet4", "Sheet1", "Sheet5"
...
// 表单顺序应该是 "Sheet1", "Sheet2", "Sheet3", "Sheet4", "Sheet5", "tableSheet1".
spread.addSheetTab(0, "tableSheet1", GC.Spread.Sheets.SheetType.tableSheet); 

// 表单顺序应该是 "Sheet1", "Sheet2", "Sheet3", "tableSheet1", "Sheet4", "Sheet5".
spread.changeSheetPosition("tableSheet1", 3); 
```
...
▸ **fromJSON**(`workbookData`, `deserializationOptions?`): `void`
...
- `ignoreFormula`（默认为 false）：在将 JSON 转换为工作簿时忽略公式。
- `ignoreStyle`（默认为 false）：在将 JSON 转换为工作簿时忽略样式。
- `frozenColumnsAsRowHeaders`（默认为 false）：在将 JSON 转换为工作簿时，将冻结的列视为行标题。
- `frozenRowsAsColumnHeaders`（默认为 false）：在将 JSON 转换为工作簿时，将冻结的行视为列标题。
- `doNotRecalculateAfterLoad`（默认为 false）：在加载 JSON 后禁止重新计算。
