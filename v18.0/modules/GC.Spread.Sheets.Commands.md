...
**`property`** {boolean} canUndo - 指示该命令是否支持撤消和重做操作

**`property`** {function} execute - 执行或撤消操作
execute方法的参数如下
...
**`param`** 调整列的大小；每个项目都是一个列对象

**`param`** 调整大小的列是否位于行标头区域

**`param`** 自适应操作是否包括表头文本

**`param`** 如果这是撤销操作，则返回`true`；否则返回`false`
...
表示在单元格编辑时，用于在进入和编辑模式之间切换编辑器状态的命令。

**`property`** {boolean} canUndo - 指示该命令是否支持撤销和重做操作。

**`property`** {function} execute - 执行执行或撤销操作。
execute 方法的参数如下。
...
**`param`** 如果这是一个撤销操作，则为 `true`；否则为 `false`。
...
**`param`** 清除单元格值范围，其项目类型为GC.Spread.Sheets.Range
...
**`param`** 源范围数组，其项目类型为GC.Spread.Sheets.Range

**`param`** 目标范围数组，其项目类型为GC.Spread.Sheets.Range

**`param`** 该操作是剪切还是复制

**`param`** 剪贴板的文本

**`param`** 指示要粘贴的内容的剪贴板粘贴选项
...
**`param`** `true`表示这是一个撤销操作；否则为`false`
...
**`param`** 如果是撤销操作，则为 `true`；否则为 `false`
...
**`param`** 是否要在复制的范围中跳过空单元格，而不替换相应的单元格

**`param`** 是否将复制数据的列更改为行，反之亦然
...
**`param`** 调整列宽；每个项目都是一个对象，其中包含 firstCol 和 lastCol

**`param`** 正在调整的列的大小

**`param`** 被调整大小的列是否在行标头区域
...
**`param`** 调整行大小；每个项目都是一个包含 firstRow 和 lastRow 属性的对象

**`param`** 表示用于调整工作表上行大小的命令

**`param`** 确定被调整大小的行是否位于列标题区域
...
**`property`** {function} execute - 执行执行或撤销操作。execute 方法的参数如下：
...
**`param`** 调整列宽；每个项目都是一个对象，其中具有 firstCol 和 lastCol

**`param`** 正在调整大小的列的尺寸

**`param`** 正在调整大小的列是否在行标题区域内
...
**`param`** 工作表名称，默认取自活动工作表。

**`param`** 表格名称，默认取自活动表格。

**`param`** 如果这是撤销操作，则为 `true`；否则为 `false`。
...
| :------ | :------ | :------ |
| `context` | [`Workbook`](../classes/GC.Spread.Sheets.Workbook.md) | 操作的执行上下文 |
| `options` | `any` | 选项 |
