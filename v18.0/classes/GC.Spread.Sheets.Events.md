...
在此工作表中对单元格进行更改时可能会发生，可能需要重绘该单元格
...
**`param`** *[GC.Spread.Sheets.SheetArea](../enums/GC.Spread.Sheets.SheetArea.md)* `sheetArea` 单击的单元格所在的工作表区域
...
**`name`** *[ClipboardChanging](GC.Spread.Sheets.Worksheet.md#ClipboardChanging)*
...
**`param`** *[GC.Spread.Sheets.Range](../classes/GC.Spread.Sheets.Range.md)* `ranges` 当前剪切或复制操作的源工作表的源范围。

**`param`** *[GC.Spread.Sheets.ClipboardActionType](../enums/GC.Spread.Sheets.ClipboardActionType.md)*  `action` 表示当前操作的类型。
...
**`param`** *object* `pasteData` 剪贴板中的数据将粘贴到工作表中
...
当对此工作表中的列或列区域进行更改，可能需要重新绘制该列或列区域时触发
...
**`param`** *[GC.Spread.Sheets.SheetArea](../enums/GC.Spread.Sheets.SheetArea.md)* `sheetArea` 工作表区域
...
在对本工作表中的列或列区域进行更改之前发生，可能需要重新绘制该列或列区域
...
**`param`** *boolean* `committed` 变化的值是否已提交
...
**`param`** *[GC.Spread.Sheets.FloatingObjects.FloatingObject](GC.Spread.Sheets.FloatingObjects.FloatingObject.md)* `floatingObject` 被移出的浮动对象
...
**`param`** *[GC.Spread.Sheets.Range](GC.Spread.Sheets.Range.md)* `[fillRange]` 当`invalidType`为`dragFill`时，用于填充操作的区域。
...
**`param`** *number* `col` 工作表列的索引已被清除筛选
...
**`param`** *number* `col` 要自动清除的工作表列的索引
...
**`param`** *[GC.Spread.Report.ReportSheet](GC.Spread.Report.ReportSheet.md)* `sheet` 触发该事件的报表。
...
**`param`** *[GC.Spread.Report.IRecord](../interfaces/GC.Spread.Report.IRecord.md)[]* `updateSuccessRecords` 成功更新或插入记录。

**`param`** *[GC.Spread.Report.IFailedRecord](../interfaces/GC.Spread.Report.IFailedRecord.md)[]* `updateFailedRecords` 更新或插入记录失败。

**`param`** *[GC.Spread.Report.IRecord](../interfaces/GC.Spread.Report.IRecord.md)[]* `deleteSuccessRecords` 删除记录成功。

**`param`** *[GC.Spread.Report.IFailedRecord](../interfaces/GC.Spread.Report.IFailedRecord.md)[]*`deleteFailedRecords` 删除记录失败。
...
当对此工作表中的行或行区域进行更改，可能需要重新绘制行或行区域时触发
...
在对此工作表中的行或行区域进行更改之前发生，可能需要重新绘制行或行区域
...
**`param`** *[GC.Spread.Sheets.Range](../classes/GC.Spread.Sheets.Range.md)[]* `oldSelections` 旧的选择区域

**`param`** *[GC.Spread.Sheets.Range](../classes/GC.Spread.Sheets.Range.md)[]* `newSelections` 新的选择区域
...
**`param`** *number* `sheetPosition` 工作表的位置, 与包含所有工作表和标签页的组合集合相关

**`param`** *boolean* `oldValue` 工作表改变了先前属性的值(isVisible, isSelected…)

**`param`** *boolean* `newValue` 工作表改变属性的值(isVisible, isSelected…)
...
**`param`** *number* `sheetIndex` 改变工作表位置
...
**`param`** *number* `oldIndex` 前一个工作表索引

**`param`** *number* `newIndex` 新的工作表索引
...
**`param`** *string* `oldValue` 工作表的旧名字

**`param`** *string* `newValue` 工作表的新名字
...
**`param`** *number* `sheetTabIndex` 用户单击的工作表选项卡的索引
...
**`param`** *number* `sheetTabIndex` 用户双击的工作表选项卡的索引
...
**`param`** *boolean* `isAfter` 是在指定的列索引之前还是之后插入列。默认值为 false，即在（指定列索引）之前插入。
...
**`param`** *boolean* `cancel` 是否取消当前的调整大小行为。默认值为 false。
