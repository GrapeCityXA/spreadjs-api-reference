...
**`deprecated`** 从17.0.0版本开始，该方法在图表插件中不可用，要使用该方法，请使用legacy-charts插件。
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | [`ColorScheme`](../modules/GC.Spread.Sheets.Charts.md#colorscheme) | that represent the chart color style. |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `value?` | [`IDataLabels`](../interfaces/GC.Spread.Sheets.Charts.IDataLabels.md) | 图表数据标签图表的样式 |

#### Returns

`any`

如果没有设置值，则返回图表的图表数据标签样式；否则，返回图表。
