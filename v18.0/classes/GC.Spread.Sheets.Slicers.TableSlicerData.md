...
清除预览过滤器状态
您可以使用 slicerData.inPreview() API 检查切片器是否在预览中进行过滤，
如果将 doFilter() 的 isPreview 标志设置为 true，（例如 slicerData.doFilter('Name', {exclusiveRowIndexes: [1]}, true);）
您可以通过clearPreview() API清除预览状态
这将删除预览过滤器状态
