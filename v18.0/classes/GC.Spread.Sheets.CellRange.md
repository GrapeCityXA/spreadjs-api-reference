...
| :------ | :------ | :------ |
| `sheet` | [`Worksheet`](GC.Spread.Sheets.Worksheet.md) | 包含此单元格区域的工作表 |
| `row` | `number` | 单元格的行索引 |
| `col` | `number` | 单元格的列索引 |
| `rowCount?` | `number` | - |
| `colCount?` | `number` | - |
...
如果未设置值，则返回该单元格是否可进入编辑模式进行编辑；否则，返回该单元格。
...
如果未设置值，则返回单元格公式是否被隐藏；否则，返回该单元格。
...
**`已弃用`** 此属性目前仅在 Internet Explorer（IE 浏览器）中有效。
...
获取或设置单元格是否锁定当工作表受到保护时，无法编辑锁定的单元格
