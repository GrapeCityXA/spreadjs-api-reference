...
| Name | Type | Description |
| :------ | :------ | :------ |
| `floatingObjectOrName` | `string` \| [`FloatingObject`](GC.Spread.Sheets.FloatingObjects.FloatingObject.md) | 将添加到工作表的浮动对象，或将添加到工作表的图片的名称 |
| `src?` | `string` | 图片的图像来源 |
| `x?` | `number` | 图片的x位置 |
| `y?` | `number` | 图片的y位置 |
| `width?` | `number` | 图片的宽度 |
