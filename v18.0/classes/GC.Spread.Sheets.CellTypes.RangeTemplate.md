...
**`description`** RangeTemplate提供了一系列参考工作表的模板，它可以应用于单元格它将使单元格与临时模板相同，并且填充数据不同如果未设置参数row,col,rowCount,colCount,则将使用整个工作表作为区域区域
...
| :------ | :------ | :------ |
| `sheet` | [`Worksheet`](GC.Spread.Sheets.Worksheet.md) | 在引用的工作表中，该工作表可以是工作簿外部的单个工作表 |
| `row?` | `number` | - |
| `col?` | `number` | - |
| `rowCount?` | `number` | - |
| `colCount?` | `number` | - |
