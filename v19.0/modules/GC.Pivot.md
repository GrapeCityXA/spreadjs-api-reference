...
**`property`** {GC.Pivot.SortType} sortType - 按升序、降序或自定义对字段进行排序

**`property`** {string} sortValueFieldName - 按指定值区域字段的值对字段进行排序

**`property`** {GC.Spread.Pivot.IPivotReference[]} sortByPivotReferences - 按指定行或列对字段进行排序。使用此属性时必须提供sortValueFieldName

**`property`** {GC.Pivot.ICustomSortCallBack} customSortCallback - 通过自定义方式对字段进行排序，应返回排序后的字段项数组

**`property`** {(string | number)[]} customSortList 用于自定义排序的自定义排序列表。
...
**`property`** {Object.<string, string[] | number[]>}  - 组字段项名称和匹配项文本。
...
**`property`** {string} sourceName 值字段的源名称。

**`property`** {GC.Pivot.SubtotalType} subtotalType 值字段的小计类型。
