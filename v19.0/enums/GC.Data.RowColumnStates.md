...
当单元格值发生变化时，单元格所在的行和列状态包含"dirty"（已修改）状态。
...
当插入行时，其状态包含"inserted"（已插入）状态。此状态仅适用于行。
...
当单元格处于选择范围内时，该单元格所在的行和列状态包含"selected"（选中）状态。
