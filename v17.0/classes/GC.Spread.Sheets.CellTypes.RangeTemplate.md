...
**`description`** RangeTemplate提供了一系列参考表单的模板，它可以应用于单元格它将使单元格与临时模板相同，并且填充数据不同如果未设置参数row,col,rowCount,colCount,则将使用整个表单作为区域区域
...
| :------ | :------ | :------ |
| `sheet` | [`Worksheet`](GC.Spread.Sheets.Worksheet.md) | 在引用的表单中，该表单可以是工作簿外部的单个表单 |
| `row?` | `number` | - |
| `col?` | `number` | - |
| `rowCount?` | `number` | - |
| `colCount?` | `number` | - |
