...
**`property`** {GC.Pivot.SortType} sortType - 通过上升或下降或自定义对字段进行排序

**`property`** {string} sortValueFieldName - 按指定值区域字段的值对字段进行排序

**`property`** {GC.Spread.Pivot.IPivotReference[]} sortByPivotReferences - 按指定的行或列对字段进行排序。如果使用此属性，则必须使用sortValueFieldName

**`property`** {GC.Pivot.ICustomSortCallBack} customSortCallback - 按自定义方式对字段进行排序。应返回有序的字段项目数组
...
**`property`** {Object.<string, string[] | number[]>}  - 组字段项目名称和匹配的项目文本
...
**`property`** {string} sourceName - 值字段源名称

**`property`** {GC.Pivot.SubtotalType} subtotalType - 值字段小型类型
