...
获取该任务下的子任务
这是内部任务的克隆数组。要修改子任务，请使用 Project.InsertTasks 和 Project.removeTasks
...
获取或设置一个浮点数，指示此任务的完成百分比。 该值应等于或大于 0
...
获取或设置此任务的持续时间
允许设置一个数字或字符串值以表示持续时间，而获取结果将始终是有效的 [GC.Spread.Sheets.GanttSheet.Duration](../modules/GC.Spread.Sheets.GanttSheet.md#duration) 对象
请注意，对于手动调度模式，持续时间可能为仅起始或完成任务的持续时间
...
获取或设置此任务的完成日期
请注意，对于手动调度模式，对于仅启动或持续时间的任务可能是无效的
在这些情况下，使用 finishDisplayed 以获取甘特图中的实际值
...
获取一个布尔值，指示此任务是正常任务。正常的任务既不是摘要任务也不是里程碑
...
获取一个布尔值，指示此任务是摘要任务。摘要任务是其他任务的父任务
...
Gets a [GC.Spread.Sheets.GanttSheet.TaskDependency](GC.Spread.Sheets.GanttSheet.TaskDependency.md) 包含此任务的所有前置任务的数组
要修改前置任务，请设置前置任务字段，或调用 addDependency 或 removeDependency
...
获取或设置一个代表此任务的前置任务的字符串值
前任字符串包含一个或多个任务编号和依赖项类型
...
获取此任务的行索引。如果任务未排序，则行索引与任务的ID相同
...
获取或设置此任务的开始日期
请注意，通过手动调度模式，对于仅完成或持续时间的任务，启动可能是无效的。在这些情况下，使用 startDisplayed 以获取甘特图中的实际值
...
获取或设置此任务的栏样式。一个任务可以在甘特图中具有多个任务栏，您可以通过指定的规则名称为每个栏设置样式
注意不要修改此值的成员，而是为此字段设置一个新的对象实例
...
Gets a [GC.Spread.Sheets.GanttSheet.TaskDependency](GC.Spread.Sheets.GanttSheet.TaskDependency.md) 包含此任务的所有继承者的数组
要修改后继者，请设置任务的继任领域，或调用 addDependency 或 removeDependency
...
获取或设置一个代表该任务后继者的字符串值
后继字符串包含一个或多个任务编号和依赖项类型
