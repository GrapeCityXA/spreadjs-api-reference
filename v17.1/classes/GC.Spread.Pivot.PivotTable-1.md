...
**`property`** {boolean} [allowMultipleFiltersPerField] 指示是否在一个字段中使用多个过滤器

**`property`** {boolean} [fillDownLabels] 指示是否显示重复标签项

**`property`** {boolean} [insertBlankLineAfterEachItem] 指示是否在每个项目末尾插入空白行

**`property`** {GC.Spread.Pivot.GrandTotalPosition} [grandTotalPosition] 指示是否在行、列或两者中显示总计

**`property`** {GC.Spread.Pivot.SubtotalsPosition} [subtotalsPosition] 指示显示小计顶部或底部或不显示

**`property`** {GC.Spread.Pivot.DisplayFields} [displayFieldsInPageFilterArea] 指示页面区域中的字段显示先上后下或先下后上

**`property`** {number} [reportFilterFieldsPerColumn] 指示每列的报表文件管理器字段数

**`property`** {boolean} [bandRows] 表示是否显示带行

**`property`** {boolean} [bandColumns] 表示是否显示带列
...
**`property`** {boolean} [showMissing] 指示missingCaption 选项是否生效

**`property`** {boolean} [missingCaption] 指示当实际值为空时应显示什么值

**`property`** {boolean} [rowLabelIndent] 表示每级标题的缩进

**`property`** {boolean} [printDrill] 在数据透视表上显示时打印展开/折叠按钮

**`property`** {boolean} [itemPrintTitles] 在每个数据透视表上重复行标签

**`property`** {boolean} [fieldPrintTitles] 设置打印标题

**`property`** {boolean} [showFilter] 表示是否显示过滤按钮

**`property`** {boolean} [showToolTip] 指示是否显示工具提示

**`property`** {boolean} [mergeItem] 指示是否将带有标签的单元格合并并居中

**`property`** {boolean} [isShowErrorValue] 指示 errorValueInfo 选项是否生效

**`property`** {boolean} [errorValueInfo] 指示当实际值错误时应显示什么值

**`property`** {string} [rowHeaderCaption] 指示应在紧凑布局中显示什么值来替换行标签

**`property`** {string} [colHeaderCaption] 指示应在紧凑布局中显示什么值来替换列标签

**`property`** {boolean} [showHeaders] 表示显示字段标题

**`property`** {GC.Spread.Pivot.CalcItemAggregation} [calcItemAggregation] 指示数据透视表总计是否包含 calcItem 的值

**`property`** {boolean} [enableDataValueEditing] 指示是否允许编辑数据透视表数据区域的单元格值
...
| `position` | [`SubtotalsPosition`](../enums/GC.Spread.Pivot.SubtotalsPosition.md) | 指示设置是否小计位置，仅支持顶部和底部 |
...
| `sourceName` | `string` | sourceField名称 |
| `calcItemName` | `string` | sourceField的calcItem名称 |
| `formula` | `string` | 此 calcItem 的新公式 |
| `priority` | `number` | 此 calcItem 的新优先级 |
